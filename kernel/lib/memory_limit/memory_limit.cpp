...
status_t mem_limit_get_iovs(mem_limit_ctx_t* ctx, uintptr_t range_base, size_t range_size,
    ...
    if (range_base <= k_base && k_base < range_end) {
        ...
        size_t r_size = ctx->ramdisk_size;
        if (r_size) {
            uintptr_t r_base = ctx->ramdisk_base;
            uintptr_t r_end = r_base + r_size;
            LTRACEF("ramdisk base %" PRIxPTR " size %" PRIxPTR "\n", r_base, r_size);
            tmp = mxtl::min(ctx->memory_limit, r_size);
            if (tmp != r_size) {
                size_t diff = r_size - ctx->memory_limit;
                printf("WARNING: ramdisk forces the system to exceed the system memory limit of %zu bytes by %zu bytes!\n",
        ...
        // Set an adjusted local limit for the cuM#M#rrent range we're scanning
        ...
        if (!ctx->found_kernel && ctx->ramdisk_size) {
